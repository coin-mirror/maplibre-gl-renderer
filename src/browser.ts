...
export class RendererBrowser {
  ...
  private async restartBrowser(): Promise<void> {
    if (this.retryCount >= this.options.maxRetries!) {
      console.error(
        `Max retry attempts (${this.options.maxRetries}) reached. Browser restart failed.`,
      );
      throw new Error(
        `Browser restart failed after ${this.options.maxRetries} attempts`,
      );
    }

    this.retryCount++;
    console.log(
      `Restarting browser (attempt ${this.retryCount}/${this.options.maxRetries})...`,
    );
    ...
    try {
      ...
      await this.initBrowser();
      console.log(
        `Browser successfully restarted (attempt ${this.retryCount})`,
      );
    } catch (error) {
      console.error(
        `Browser restart attempt ${this.retryCount} failed:`,
        error,
      );
      if (this.retryCount < this.options.maxRetries!) {
        // Try again
  ...
  public async close(): Promise<void> {
    ...
    if (this.restartPromise) {
      try {
        ...
        console.warn(
          "Error waiting for browser restart during shutdown:",
          error,
        );
      }
    }
