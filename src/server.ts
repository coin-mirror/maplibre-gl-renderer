...
class MapScreenshotServer {
  ...
  private getNextRendererIndex(): number {
    ...
    const startIndex = this.currentRendererIndex;
    let attempts = 0;
    
    do {
      this.currentRendererIndex = (this.currentRendererIndex + 1) % this.rendererCount;
      attempts++;
  ...
  private addToRenderQueue(
    ...
    return new Promise((resolve, reject) => {
      ...
      this.renderQueue.add(() => this.processRenderTask(task), {
        priority: 0,
      }).catch(reject);
    });
  }
  ...
  private async processRenderTask(task: RenderTask): Promise<void> {
    ...
    if (task.signal.aborted || this.isShuttingDown) {
      return task.reject(new Error("Task was aborted or service is shutting down"));
    }
    ...
    try {
      ...
      if (this.isCriticalError(errorMessage)) {
        ...
        this.reinitializeRenderer(rendererIndex).catch((err) => {
          console.error(`Failed to reinitialize renderer ${rendererIndex}:`, err);
        });
      }
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      const initPromises = [];
      for (let i = 0; i < this.rendererCount; i++) {
        initPromises.push(this.initRenderer(i).catch((error) => {
      ...
      const healthyRenderers = this.renderers.filter(r => r !== null).length;
      console.log(`Server started with ${healthyRenderers}/${this.rendererCount} healthy renderers`);
